---
id: WOF-SUPPLY-006
title: "Dynamic Package Installation or Unpinned Dependency Resolution in CI"
kind: vulnerability
...
detection:
  ...
  candidate_tokens:
    ...
    - "~"
indicators:
  - CI steps that run `npm install <pkg>@latest`, `npm i <pkg>@latest`, `npx <pkg>`, or `npm exec` against packages that are not already pinned and locked in the repository
  - workflows that regenerate or rewrite `package-lock.json`, or that run `npm install` from dependency ranges such as `^`, `~`, `*`, dist-tags, or mutable git refs
  ...
  - pull requests that widen dependency specs, remove exact pins, or replace `npm ci` with mutable install flows in trusted automation
tags:
  - supply-chain
  - npm
  - npx
...
Do not let build, release, signing, or publish jobs dynamically fetch tools via `npx`, `npm exec`, or `npm install ...@latest`, and do not let CI rebuild dependency trees from loose ranges unless the exact versions and lockfile diff are reviewed and committed first.
...
CI workflow step
      ↓
live registry lookup or range resolution
      ↓
...
If a workflow runs `npm install some-tool@latest`, `npx some-tool`, or regenerates dependencies from `^` or `~` ranges, the resulting build can change without any new repository diff.
...
jobs:
  publish:
    ...
    steps:
      ...
      - name: Install dependencies from version ranges
        run: npm install
...
1. The workflow allows runtime dependency resolution from npm instead of consuming only reviewed exact versions from source control.
2. `npm install <pkg>@latest` and dependency specs such as `^1.2.3` or `~1.2.3` permit newer published versions to be selected later.
...
Pin exact dependency versions in `package.json`, commit `package-lock.json`, use `npm ci` in CI, and perform dependency refreshes only in reviewed changesets that commit the resulting manifest and lockfile updates together.
...
jobs:
  publish:
    ...
    steps:
      ...
      - name: Publish release notes with the pinned local tool
...
- Candidate collection: inspect `package.json` diffs for dependency specs beginning with `^`, `~`, `*`, dist-tags such as `latest`, or mutable git refs when those dependencies are installed or refreshed in CI.
...
- Confirmation: verify that the workflow resolves package versions from npm at runtime, or rebuilds the dependency tree from mutable ranges, and that the resolved package can execute code or materially influence a trusted build, release, signing, deployment, or artifact output.
...
- `npx` or `npm exec` that only invokes a local already-pinned tool from the repository's reviewed dependency tree is lower risk than remote resolution from the registry.
- A dependency update PR that pins exact versions and commits the lockfile for review is not this rule, even though it intentionally changes package versions.
...
The same underlying issue appears in any package manager when trusted automation resolves packages from live registry state or rebuilds lockfiles from mutable ranges during the workflow itself.
...
- Dependencies executed in CI are pinned to exact reviewed versions.
